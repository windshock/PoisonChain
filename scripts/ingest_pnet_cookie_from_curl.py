#!/usr/bin/env python3
"""Read curl text from stdin or file; extract -b $'...' cookie; merge PNET_COOKIE into repo .env."""
import re
import sys
...
def merge_env(cookie: str) -> None:
    ...
    pnet_line = f'PNET_COOKIE="{esc}"\n'
    raw = ENV_PATH.read_text(encoding="utf-8-sig") if ENV_PATH.exists() else ""
    out = []
    replaced = False
    for line in raw.splitlines(keepends=True):
        if line.startswith("PNET_COOKIE="):
            if not replaced:
                out.append(pnet_line)
                replaced = True
            continue
        out.append(line)
    if not replaced:
...
def main():
    ...
    merge_env(cookie)
    print(f"OK: wrote PNET_COOKIE to .env ({len(cookie)} chars)")
