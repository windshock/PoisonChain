...
def team_label_from_committer(c: dict) -> str:
    """조직 **팀**만: pnet 부서(유효값) → Bitbucket 표시명에서 팀. (Bitbucket 프로젝트 키는 팀이 아님.)"""
    d = (c.get("pnet_dept") or "").strip()
    if d and "@" not in d:
        return d
...
def dominant_org_team_per_project(repos: List[dict]) -> Dict[str, Optional[str]]:
    ...
    **미분류가 아닌 팀** 출현 빈도가 가장 많은 값을 '프로젝트 대표 팀'으로 쓴다.
    (사번-only·pnet 부서 없음인 담당자는 이 팀으로 묶어 상위 그룹과 맞춘다.)
    """
...
def main():
    ...
    w("")
    w(
        "**팀(조직):** 커밋자별로 `pnet_dept` → Bitbucket 표시명: 마지막이 **SKP** 등이면 "
