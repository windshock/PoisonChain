...
def merge_pnet_enrichment(old_committers, new_committers):
    """Re-fetch loses employee_status / pnet_* from check_employee_status.py — restore by email."""
...
def main():
    ...
    def fetch(repo):
        ...
        committers = merge_pnet_enrichment(repo.get("committers"), committers)
...
def generate_report(scan, results):
    ...
    w("")
    w("| # | 리포지토리 | axios(lock) | package.json | semver 위험 | lockfile | pnet 부서(있으면) | 최근 커밋 담당자 |")
    w("|---|---|---|---|---|---|---|---|")
