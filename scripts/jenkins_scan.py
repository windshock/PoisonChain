...
internal/reports/data/bitbucket-full-scan-result.json의 axios semver 위험 리포와 매칭하여
...
출력: internal/reports/data/jenkins-scan-result.json
"""
...
INTERNAL_CONFIG_DIR = ROOT_DIR / "internal" / "config"
REPORTS_DATA_DIR = ROOT_DIR / "internal" / "reports" / "data"

INSTANCES_JSON  = str(INTERNAL_CONFIG_DIR / "jenkins-instances.json")
BITBUCKET_JSON  = str(REPORTS_DATA_DIR / "bitbucket-full-scan-result.json")
OUTPUT_JSON     = str(REPORTS_DATA_DIR / "jenkins-scan-result.json")
AUTH_CACHE_FILE = str(REPORTS_DATA_DIR / "jenkins-auth-cache.json")   # url → user:token
SCAN_DONE_FILE  = str(REPORTS_DATA_DIR / "jenkins-scanned-urls.json") # 스캔 완료 URL 목록
...
# 토큰 → 인증 성공 여부 캐시 (base_url → auth_header)
_auth_cache: dict[str, str] = {}


def _test_auth(base_url: str, auth_header: str) -> bool:
    """해당 auth로 /api/json 접근 가능한지 확인."""
    try:
        http_get(base_url.rstrip("/") + "/api/json?tree=nodeName", auth_header=auth_header, timeout=5)
        return True
    except Exception:
        return False


def _build_auth(token: str, base_url: str) -> str:
    """토큰(또는 user:token)으로 Authorization 헤더 생성. username 자동 조회."""
    if ":" in token:
        user, pw = token.split(":", 1)
        return _basic_auth_header(user, pw)
    # JENKINS_USERNAMES(사원번호 등) + 기본 후보 순서로 시도
    extra = [u.strip() for u in os.environ.get("JENKINS_USERNAMES", "").split(",") if u.strip()]
    candidates = extra + ["admin", "jenkins", "user"]
    for candidate in candidates:
        auth = _basic_auth_header(candidate, token)
        try:
            raw = http_get(base_url.rstrip("/") + "/me/api/json?tree=id", auth_header=auth, timeout=5)
            username = json.loads(raw).get("id", "").strip()
            if username:
                return _basic_auth_header(username, token)
        except Exception:
            continue
    return _basic_auth_header("jenkins", token)


def resolve_auth(instance_id: int, lab: bool = False, base_url: str = "") -> str | None:
    """
    Authorization 헤더 반환. 우선순위:
    0. jenkins-auth-cache.json   — 이전 매칭 결과 파일 캐시
    1. JENKINS_TOKEN_{id}        — 특정 인스턴스 전용 토큰
    2. JENKINS_TOKEN             — 단일 인스턴스용 토큰
    3. JENKINS_TOKENS            — 쉼표 구분 토큰 뭉터기 → 서버마다 자동으로 맞는 토큰 탐색
    4. lab admin                 — --lab 모드 전용

    토큰 형식: 'user:token' 또는 토큰만 (username 자동 조회).
    """
    # 메모리 캐시 확인
    if base_url and base_url in _auth_cache:
        return _auth_cache[base_url]

    # 0. auth-cache 파일 (이전 실행에서 저장된 결과)
    if base_url and os.path.exists(AUTH_CACHE_FILE):
        try:
            file_cache: dict = json.loads(open(AUTH_CACHE_FILE, encoding="utf-8").read())
            cred = file_cache.get(base_url, "")
            if cred:
                auth = _build_auth(cred, base_url)
                _auth_cache[base_url] = auth
                return auth
        except Exception:
            pass
    ...
    if token:
        auth = _build_auth(token, base_url)
        if base_url:
            _auth_cache[base_url] = auth
        return auth

    # 2. 토큰 뭉터기 (JENKINS_TOKENS) — 맞는 토큰 자동 탐색
    tokens_raw = os.environ.get("JENKINS_TOKENS", "").strip()
    if tokens_raw and base_url:
        candidates = [t.strip() for t in tokens_raw.split(",") if t.strip()]
        for t in candidates:
            auth = _build_auth(t, base_url)
            if _test_auth(base_url, auth):
                print(f"  🔑 [{base_url}] 토큰 자동 매칭 성공")
                _auth_cache[base_url] = auth
                return auth
        print(f"  ⚠️  [{base_url}] 매칭되는 토큰 없음 — 익명 접근 시도")
        return None
...
def scan_instance(instance: dict, bb_index: dict[str, dict], lab: bool = False) -> dict:
    ...
    auth = resolve_auth(iid, lab=lab, base_url=base_url)
...
def main() -> None:
    ...
    if os.path.exists(SCAN_DONE_FILE) and not instance_url and not lab:
        try:
            done_urls = set(json.loads(open(SCAN_DONE_FILE, encoding="utf-8").read()))
            if done_urls:
                print(f"⏭️  이전 스캔 완료 서버 {len(done_urls)}대 skip")
        except Exception:
            pass
    skip_done = "--rescan" not in args  # --rescan 플래그 시 강제 재스캔

    # --only-cached: auth-cache에 있는 서버만 스캔 (토큰 없는 서버 즉시 제외)
    only_cached = "--only-cached" in args
    if only_cached and os.path.exists(AUTH_CACHE_FILE):
        try:
            cached_urls = set(json.loads(open(AUTH_CACHE_FILE, encoding="utf-8").read()).keys())
            before = len(instances)
            instances = [i for i in instances if i["url"].rstrip("/") in cached_urls]
            print(f"🔑 --only-cached: {len(instances)}대 (전체 {before}대 중 인증 가능 서버만)")
        except Exception:
            pass

    instances_to_scan = [i for i in instances
                         if not skip_done or i["url"].rstrip("/") not in done_urls]
    if len(instances_to_scan) < len(instances):
        print(f"   ({len(instances) - len(instances_to_scan)}대 이미 완료, {len(instances_to_scan)}대 대상)")
    ...
    max_workers = min(10, len(instances_to_scan))
    newly_done: list[str] = []
    with ThreadPoolExecutor(max_workers=max(1, max_workers)) as executor:
        futures = {executor.submit(scan_instance, inst, bb_index, lab): inst for inst in instances_to_scan}
        for future in as_completed(futures):
            ...
            if res["skipped"]:
                ...
                newly_done.append(res["instance_url"])

    # 스캔 완료 URL 파일 업데이트
    if newly_done and skip_done:
        all_done = done_urls | set(newly_done)
        REPORTS_DATA_DIR.mkdir(parents=True, exist_ok=True)
        with open(SCAN_DONE_FILE, "w", encoding="utf-8") as f:
            json.dump(sorted(all_done), f, indent=2)
        print(f"💾 완료 목록 저장: {SCAN_DONE_FILE} ({len(all_done)}대 누적)")
    ...
    if only_cached:
        partial_scan_reasons.append("auth-cache 에 있는 인스턴스만 대상으로 제한")
    if skip_done and len(instances_to_scan) < len(instances):
        partial_scan_reasons.append("이전에 완료된 URL 을 건너뜀")
    if skipped:
    ...
    output = {
        ...
        "instances_targeted": len(instances_to_scan),
        "instances_scanned": scanned,
        "instances_skipped": skipped,
        "partial_scan": partial_scan,
        "partial_scan_reasons": partial_scan_reasons,
        "total_jobs_found": len(all_jobs),
        # matched_repos is the unique Bitbucket repo count; matched_jobs remains
        # available in the summary when multiple jobs map to one repo.
    ...
    print(f"\n📄 결과: {OUTPUT_JSON}")
    print(f"   인벤토리: {inventory_count}, 이번 대상: {len(instances_to_scan)}")
