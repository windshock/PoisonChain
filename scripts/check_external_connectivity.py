...
tracert.example.com tcping API를 사용해 Jenkins 인스턴스 및 서비스 서버가
...
Usage:
  ...
  --nat-only   : internal CMDB에서 NAT 할당 서버를 자동 조회해 스캔 (권장)
  --jenkins-only: Jenkins 인스턴스 전체 스캔 (NAT 무관)
  --dry-run    : 실제 요청 없이 대상 목록만 출력

Environment:
    HR_PORTAL_ID / HR_PORTAL_PW  사내 포털 계정 — tracert/cmdb 자동 로그인에 사용
    TRACERT_COOKIE     (선택) 수동 세션 쿠키 (없으면 HR_PORTAL_ID/PW로 자동 로그인)
"""
...
INFRASE_BASE       = "https://infra.example.com"
...
TRACERT_URL = "https://tracert.example.com/tracert.php"
...
def login_tracert() -> str:
    """HR_PORTAL_ID / HR_PORTAL_PW로 tracert.example.com에 로그인해 PHPSESSID를 반환."""
    portal_id = os.environ.get("HR_PORTAL_ID", "")
    portal_pw = os.environ.get("HR_PORTAL_PW", "")
    if not portal_id or not portal_pw:
        return ""

    data = urllib.parse.urlencode({"id": portal_id, "password": portal_pw}).encode()
    req = urllib.request.Request(
        "https://tracert.example.com/login.php",
        data=data,
        headers={
...
def get_cookie() -> str:
    ...
    # TRACERT_COOKIE 없으면 HR_PORTAL_ID/HR_PORTAL_PW로 자동 로그인 시도
    portal_id = os.environ.get("HR_PORTAL_ID", "")
    if portal_id:
        print(f"🔑 TRACERT_COOKIE 없음 — HR_PORTAL_ID({portal_id})로 자동 로그인 시도...")
    ...
    print("❌ TRACERT_COOKIE 또는 HR_PORTAL_ID/HR_PORTAL_PW가 없습니다.")
    print("   .env에 HR_PORTAL_ID=xxx 와 HR_PORTAL_PW=yyy 를 추가하세요.")
    sys.exit(1)


def tcping(sip: str, dip: str, dport: str, cookie: str, dry_run: bool) -> bool | None:
    """
    tracert.example.com으로 sip → dip:dport tcping 요청.
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "Referer": "https://tracert.example.com/",
            "Origin": "https://tracert.example.com",
        }
    )
    try:
...
def login_cmdb() -> str:
    """HR_PORTAL_ID/HR_PORTAL_PW로 infra.example.com에 로그인해 PHPSESSID를 반환."""
    portal_id = os.environ.get("HR_PORTAL_ID", "")
    portal_pw = os.environ.get("HR_PORTAL_PW", "")
    if not portal_id or not portal_pw:
        return ""
...
def fetch_nat_from_cmdb(ips: list[str], cmdb_session: str, today: str) -> dict[str, dict]:
    """
    internal CMDB에서 IP 목록의 NAT 정보를 일괄 조회.
    Returns: {ip: {hostname, zone, nat_public_ip, service_name, ...}}
    """
    if not cmdb_session or not ips:
        return {}
    ...
    req = urllib.request.Request(
        ...
        headers={
            "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8",
            "Cookie": f"PHPSESSID={cmdb_session}",
            "X-Requested-With": "XMLHttpRequest",
            "User-Agent": "Mozilla/5.0",
            "Origin": INFRASE_BASE,
...
def fetch_contact(ip: str, cmdb_session: str) -> dict:
    """
    internal CMDB에서 서버 IP의 개발담당 연락처 + hostname + 서비스명 조회.
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "Mozilla/5.0", "Cookie": f"PHPSESSID={cmdb_session}"},
    )
    try:
...
def enrich_contacts(results: list[dict], cmdb_session: str) -> list[dict]:
    ...
    for r in reachable:
        contact = fetch_contact(r["ip"], cmdb_session)
        r["contact"] = contact
...
def load_nat_targets(cmdb_session: str = "") -> list[dict]:
    """
    internal CMDB에서 모든 Jenkins IP의 NAT 할당 여부를 자동 조회.
    ...
    print("🔍 internal CMDB에서 NAT 할당 정보 자동 조회 중...")
    if not cmdb_session:
        portal_id = os.environ.get("HR_PORTAL_ID", "")
        if portal_id:
            cmdb_session = login_cmdb()

    if cmdb_session:
        nat_map = fetch_nat_from_cmdb(all_ips, cmdb_session, today)
        print(f"  ✅ CMDB 응답: {len(nat_map)}개 NAT 서버 확인")
    else:
        print("  ⚠️  CMDB 로그인 실패 — nat-inventory.json 사용")
        nat_map = {}
    ...
    # nat-inventory.json 갱신 (cmdb에서 실제로 조회한 경우만)
    if nat_map and cmdb_session:
        servers = []
...
def main():
    ...
    # cmdb 세션 (NAT 조회 + 담당자 조회 공용)
    cmdb_session = ""
    portal_id = os.environ.get("HR_PORTAL_ID", "")
    if portal_id and not dry_run:
        cmdb_session = login_cmdb()

    if nat_only:
        targets = load_nat_targets(cmdb_session)
    else:
    ...
    if not dry_run and cmdb_session:
        results = enrich_contacts(results, cmdb_session)
