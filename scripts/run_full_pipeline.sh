...
if [[ "$HR_REFRESH" == "1" ]]; then
  echo "==> hr_portal만: check_employee_status.py (기존 bitbucket-full-scan-result.json 갱신)"
  $PY scripts/check_employee_status.py
  echo "==> hr_portal만: report_axios_by_team.py"
...
if [[ "$WITH_HR" == "1" ]]; then
  echo "==> [6] check_employee_status.py (HR_PORTAL_COOKIE 필요)"
  if $PY scripts/check_employee_status.py; then
    echo "==> [7] fetch_committers.py (2차, hr_dept + manual-overrides MD 반영)"
    $PY scripts/fetch_committers.py
  else
    echo "WARN: 재직 조회 실패(.env 의 HR_PORTAL_COOKIE 없음·만료·망 등). 나머지는 계속." >&2
  fi
fi
