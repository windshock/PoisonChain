...
- `./scripts/run_full_pipeline.sh` — `internal/docs/PROJECT-OVERVIEW.md` §4와 동일 순서(옵션: `--with-hr`, `--with-lockfile`). Cursor 등은 동일 내용 **`AGENTS.md`** 도 참고할 수 있다.
...
- **재직 조회:** `HR_PORTAL_COOKIE`는 `.env`에만 둔다(값을 설명/커밋에 넣지 말 것).
- **변경 범위:** 요청한 작업에 필요한 파일만 수정한다. 불필요한 대규모 리팩터·무관한 문서 생성은 하지 않는다.

## 환경

- `.env` 예: `XEIZE_API_KEY`, `XEIZE_BASE_URL`; 재직 조회 스크립트는 **`HR_PORTAL_COOKIE`**(HR 포털 세션, `curl -b` 형식). 샘플은 `public/.env.example` 참고.
